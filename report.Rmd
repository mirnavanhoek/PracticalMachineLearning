...
```
In figures 1-3 in the Appendix the confusion matrices for the different models are plotted. As can be seen the Random Forest model performs the best/has the best predictions, i.e. is the most accurate since most observations in the confusion matrix end up on the diagonal and less observations end up on the off-diagonal (so smaller out of sample error).
Based on the accuracy results the Random Forest Model, with an accuracy of `r cmRF$overall[1]`, has the highest accuracy (and thus the smalles out of sample error (`r 1-cmRF$overall[1]`)), therefore, the Random Forest Model is the best model.
...
The more observations are on the diagonal of the confusion matrix the more accurate the model predicts.
The less observations are on the off-diagonal of the confusion matrix the lower the out of sample error is.
